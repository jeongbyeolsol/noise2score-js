...
def build_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path for the integrated Noise2Score checkpoint. Defaults to output_dir/noise2score_checkpoint.pt.",
    )
...
def force_noise_type(args, forced_noise_type):
    ...
    if args.noise_type != forced_noise_type:
        tqdm.write(
            f"[noise2score] forcing --noise-type {forced_noise_type} "
...
def run(args, forced_noise_type=None):
    ...
    checkpoint_output = (
        ...
        else output_dir / "noise2score_checkpoint.pt"
    )
    save_noise2score_checkpoint(
        checkpoint_output,
        n2s,
        ardae_checkpoint=ardae_ckpt,
        ardae_checkpoint_path=checkpoint_path,
        args=args,
        blind=False,
    )
